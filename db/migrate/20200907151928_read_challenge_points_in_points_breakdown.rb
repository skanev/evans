class ReadChallengePointsInPointsBreakdown < ActiveRecord::Migration
  def build_view_sql(challenges_sql)
    <<-SQL
  ...
  def up
    ...
    execute build_view_sql(<<-SQL)
      SELECT
        users.id AS user_id,
        SUM(COALESCE(challenges.points, 0)) AS points
      FROM users
