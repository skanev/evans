class AddCommentStarsToPointsBreakdown < ActiveRecord::Migration
  def change
    ...
    execute <<-SQL
      ...
      WITH users_tasks AS (
        ...
        SELECT
          users.id AS user_id,
          COUNT(posts.starred) + COUNT(comments.starred) AS points
        FROM users
        LEFT JOIN posts ON posts.user_id = users.id AND posts.starred
        LEFT JOIN comments ON comments.user_id = users.id AND comments.starred
        GROUP BY users.id
      ), vouchers_summary AS (
        SELECT
          users.id AS user_id,
