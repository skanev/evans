class MakeSolutionPointsNull < ActiveRecord::Migration
  def up
    execute 'DROP VIEW points_breakdowns'
    change_column :solutions, :points, :integer, null: true, default: nil
    execute <<-SQL
      ...
      SELECT
        rank() OVER (ORDER BY arrays_summary.tasks + arrays_summary.quizzes + stars_summary.points + vouchers_summary.points + challenges_summary.points + photos_summary.points DESC) AS rank,
        users.id AS id,
        users.name AS name,
        ...
        arrays_summary.tasks + arrays_summary.quizzes + stars_summary.points + vouchers_summary.points + challenges_summary.points + photos_summary.points AS total
      FROM users
