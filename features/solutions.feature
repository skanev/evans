# language: bg
Функционалност: Решения
  За да могат студентите да учат един от друг
  като преподаватели
  искаме да могат да разглеждат решенията си и да обменят идеи върху тях чрез коментари

  Сценарий: Разглеждане на решения
    Дадено че "Първа задача" има следните решения:
      | Студент      | Успешни | Неуспешни | Код             |
      | Петър Петров | 12      | 6         | print("larodi") |
      | Иван Иванов  | 18      | 0         | print("baba")   |
    Когато отида на решенията на "Първа задача"
    То трябва да виждам следните решения:
      | Име          | Изпълнение                                                         |
      | Петър Петров | Успешни тестове: 12, Неуспешни тестове: 6, Редове: 1, Коментари: 0 |
      | Иван Иванов  | Успешни тестове: 18, Неуспешни тестове: 0, Редове: 1, Коментари: 0 |

  Рамка на сценарий: Изчисляване на точки за решение
    Дадено че "Първа задача" има следните решения:
      | Студент      | Успешни   | Неуспешни   | Код             |
      | Петър Петров | <Успешни> | <Неуспешни> | print("larodi") |
    Когато отида на решенията на "Първа задача"
    То трябва да виждам едно решение с "<Точки>" точки

    Примери:
      | Успешни | Неуспешни | Точки |
      | 18      | 0         | 6     |
      | 17      | 1         | 6     |
      | 16      | 2         | 5     |
      | 12      | 6         | 4     |
      | 0       | 18        | 0     |

  Сценарий: Разглеждане на резултати на отворена задача
    Дадено че съм студент
    И че има отворена задача "Първа задача"
    Когато отида на решенията на "Първа задача"
    То трябва да виждам "Нямате достъп до тази страница"

  Сценарий: Разглеждане на решение
    Дадено че "Първа задача" има следните решения:
      | Студент      | Успешни | Неуспешни | Код             | Лог |
      | Петър Петров | 12      | 6         | print('larodi') | OK  |
    Когато отида на решението на "Петър Петров" за "Първа задача"
    То трябва да виждам "12 успешни тест(а)"
    И трябва да виждам "6 неуспешни тест(а)"
    И трябва да виждам "print('larodi')"
    И трябва да виждам "OK"

  Сценарий: Даване на бонус точки на решение
    Дадено че съм влязъл като администратор
    И че студент "Петър Петров" е предал решение на задача "Класика и джаз"
    Когато дам 2 бонус точки на решението на "Петър Петров"
    То решението на "Петър Петров" трябва да има 2 допълнителни точки

  Сценарий: показване на RSS Feed линк
    Дадено че съм студент
    И че има затворена задача "Първа задача"
    Когато отида на решенията на "Първа задача"
    То трябва да виждам картинка с алтернативен текст "Абонирайте се за RSS Feed към тази страница"

  Сценарий: отваряне на RSS линк
    Дадено че съм студент
    И че има затворена задача "Първа задача"
    Когато отида на решенията на "Първа задача"
    И проследя "Абонирайте се за RSS Feed към тази страница"
    То трябва да съм на RSS адреса на решенията на "Първа задача"

  Сценарий: достъп до RSS Feed
    Дадено че съм студент
    И че има отворена задача "Първа задача"
    Когато отида на RSS адреса на решенията на "Първа задача"
    То трябва да виждам "Нямате достъп до тази страница"

  Сценарий: показване на RSS Feed
    Дадено че "Първа задача" има следните решения:
      | Студент      | Успешни | Неуспешни | Код             |
      | Петър Петров | 12      | 6         | print("larodi") |
      | Иван Иванов  | 18      | 0         | print("baba")   |
    И следните коментари към решението на "Петър Петров"
      | Студент       | Коментар         |
      | Джаста Праста | става            |
      | Иван Иванов   | аууу, много зле! |
    И следните коментари към решението на "Иван Иванов"
      | Студент       | Коментар         |
      | Джаста Праста | Брау!            |
    Когато отида на RSS адреса на решенията на "Първа задача"
    То трябва да виждам "Програмиране с Python :: Решения на Първа задача"
    И трябва да виждам следните feeds:
      | Заглавие                                              | Тяло             |
      | Коментар към решение на Петър Петров от Джаста Праста | става            |
      | Коментар към решение на Петър Петров от Иван Иванов   | аууу, много зле! |
      | Коментар към решение на Иван Иванов от Джаста Праста  | Брау!            |
